...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
  ...
  <!-- Search and Filter -->
<form method="GET" action="{{ url('/admin/account/useraccount') }}" class="flex flex-col md:flex-row items-center mb-4 gap-2 w-full">
  ...
  <div class="flex gap-2">
    ...
    <a href="{{ url('/admin/account/useraccount') }}" class="bg-red-500 hover:bg-red-600 text-white font-bold py-2 px-4 rounded">
      Clear
    </a>
  </div>
  ...
  <thead>
    <tr class="bg-blue-900 text-white">
      ...
      <th class="px-2 py-1 border-b text-left">Department</th>
  ...
  <tbody>
    @forelse($usermodel->where('user_type_id', '!=', 0) as $data)
      <tr class="bg-white border-b hover:bg-gray-100">
        ...
        <td class="px-2 py-1">{{ $data->middlename }}</td>
        ...
        <td class="px-2 py-1">{{ $data->status }}</td>
        <td class="px-2 py-1">
    ...
    @empty
      <tr>
        <td colspan="9" class="text-center py-3 text-gray-500 text-sm">No users found.</td>
      </tr>
    @endforelse
  </tbody>
</table>
 <div class="d-flex justify-content-center mt-4">
              {{ $usermodel->links('pagination::tailwind') }}
          </div>
       </div>
     </div>
