  ...
  <div class="flex min-h-screen bg-gray-100 w-full">
    ...
    <div id="mainContent" class="md:ml-64 md:flex">
          ...
          <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-6 mx-auto">
            <h2 class="text-2xl font-bold mb-6 text-center">Position</h2>

        <form action="{{ route('admin.setup.storeposition') }}" method="POST" enctype="multipart/form-data" class="max-w-xl mx-auto p-6 bg-white rounded-lg shadow-md">
            @csrf
              <div class="mb-4">
                <label class="block text-gray-700 font-bold mb-2" for="position">Position</label>
                <input class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" type="text" id="position" name="position" required>
              </div>
   ...
   <!-- Search and Filter -->
 <form method="GET" action="{{ url('/admin/setup/position') }}" class="flex flex-col md:flex-row justify-between items-center mb-4 gap-2">
      ...
      @foreach($positionmodel as $data)
        <tr class="bg-white odd:bg-gray-100 hover:bg-gray-200">
          ...
          <td class="px-4 py-2 border-b space-x-4">
            <a href="{{ route('deleteposition', $data->id) }}"
